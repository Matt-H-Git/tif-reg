...
def exp(x):
    ...
    R = I + sinc1_t*W + sinc2_t*S
    #R = I + sinc1_t.matmul(W) + sinc2_t.matmul(S)

    #V = sinc1(t)*eye(3) + sinc2(t)*W + sinc3(t)*(w*w')
    #  = eye(3) + sinc2(t)*W + sinc3(t)*S
    V = I + sinc2_t*W + sinc3_t*S
    #V = I + sinc2_t.matmul(W) + sinc3_t.matmul(S)
    ...
    #g_shape = torch.cat((torch.as_tensor(x.size()[0:-1]), torch.tensor([4, 4])), 0)
    #return g.view(*(x.size()[0:-1]), 4, 4)

    g_shape = list(x.size()[0:-1])  # SCRIPTING:
    g_shape.extend((4, 4))
...
def inverse(g):
    ...
    ig_shape = torch.cat((torch.as_tensor(g.size()[0:-2]), torch.tensor([4, 4])), 0)  # SCRIPTING:
    #return ig.view(*(g.size()[0:-2]), 4, 4)
    return ig.view(ig_shape)
...
def transform(g, a):
    ...
    # g_shape = (torch.as_tensor(g.size()[0:-2]))  # SCRIPTING:


    #R = g_[:, 0:3, 0:3].contiguous().view(*(g.size()[0:-2]), 3, 3)
    #p = g_[:, 0:3, 3].contiguous().view(*(g.size()[0:-2]), 3)

    g_shape = list(g.size()[0:-2])  # SCRIPTING:
    g_shape.extend((3, 3))

    R = g_[:, 0:3, 0:3].contiguous().view(g_shape)  # SCRIPTING:

    g_shape_1 = list(g.size()[0:-2])  # SCRIPTING:
    g_shape_1.append(3)
    p = g_[:, 0:3, 3].contiguous().view(g_shape_1)  # SCRIPTING:

    if len(g.size()) == len(a.size()):
        b = R @ a + p.unsqueeze(-1)
    else:
        b = (R @ a.unsqueeze(-1)).squeeze(-1) + p
...
# For converting the above into a format allowed by TorchScript
# https://discuss.pytorch.org/t/does-torchscript-support-custom-autograd-function/58540
